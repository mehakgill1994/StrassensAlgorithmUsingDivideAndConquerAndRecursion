...
SMatrix<int> add(SMatrix<int> P, SMatrix<int> Q)
{
	int s = P.size();
	SMatrix<int> result(s);
	for(int i=0; i<s; i++)
	{
		for(int j=0; j<s; j++)
		{
...
SMatrix<int> subtract(SMatrix<int> P, SMatrix<int> Q)
{
	int s = P.size();
	SMatrix<int> result(s);
	for(int i=0; i<s; i++)
...
SMatrix<int> partition(SMatrix<int> M, int startx, int starty, int endx, int endy)
{
	int s = M.size();
	SMatrix<int> P (s/2);
...
SMatrix<int> combine(SMatrix<int> P, SMatrix<int> Q, SMatrix<int> R, SMatrix<int> S)
{
	int s = P.size();
	SMatrix<int> M(2*s);
	for(int i=0; i<s; i++)
...
SMatrix<int> function(SMatrix<int> A,SMatrix<int> B)
{
	int n = A.size();
	int r;
	SMatrix<int> c(n), S1(n/2), S2(n/2), S3(n/2), S4(n/2), S5(n/2), S6(n/2), S7(n/2), S8(n/2), S9(n/2), S10(n/2), P1(n/2), P2(n/2), P3(n/2), P4(n/2), P5(n/2), P6(n/2), P7(n/2), c00(n/2), c01(n/2), c10(n/2), c11(n/2);	
	if(A.size() == 1)
	{
		c(0,0) = A(0,0) * B(0,0);
	}	
